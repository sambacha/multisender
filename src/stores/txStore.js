...
class TxStore {
  ...
  async doSend() {
    ...
    this.approval = '';
    if (
      new BN(this.tokenStore.totalBalance).gt(new BN(this.tokenStore.allowance))
    ) {
      this._approve();
  ...
  async _multisend({ slice, addPerTx }) {
    ...
    try {
      let encodedData = await multisender.methods
        .multisendToken(token_address, addresses_to_send, balances_to_send)
      ...
      let tx = multisender.methods
        ...
        .on('transactionHash', (hash) => {
          ...
          From ${addresses_to_send[0]} to: ${
              addresses_to_send[addresses_to_send.length - 1]
            }
        `,
            hash,
          });
